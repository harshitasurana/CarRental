...
const Cars = () => {
  ...
  const isSearchData= pickupLoc && pickupDate && returnDate
  ...
  const searchCarAvailability = async () =>{
    try{
      const {data}= await axios.post('/api/bookings/check-availability',{location:pickupLoc,pickupDate,returnDate})
      if(data.success){
  ...
  return (
    <div className='mb-14'>
      <div className='flex flex-col items-center py-20 bg-light max-md:px-4'>
        <Title title='Available Cars' subTitle='Browse our selection of vehicles available for your next adventure'/>
